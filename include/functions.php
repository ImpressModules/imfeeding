 ...
 */
function imfeeding_getModuleAdminLink($moduleName='imfeeding') {
	global $xoopsModule;
	...
	$ret = '';
	if ($moduleName) {
		$ret = "<a href='" . ICMS_URL . "/modules/$moduleName/admin/index.php'>" ._MD_IMFEEDING_ADMIN_PAGE . "</a>";
	}
	return $ret;
}
 ...
 * @todo to be move in icms core
 */
function imfeeding_getModuleName($withLink = true, $forBreadCrumb = false, $moduleName = false) {
	if (!$moduleName) {
		global $xoopsModule;
		$moduleName = $xoopsModule->getVar('dirname');
	}
 ...
 */
function imfeeding_getPreviousPage($default=false) {
	global $impresscms;
 ...
 */
function imfeeding_getMonthNameById($month_id) {
